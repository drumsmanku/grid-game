...
const Grid = () => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      for (let j = 2; j < 6; j++) initialGrid[i][j] = Math.random() < 0.5 ? 'blue' : null;
      for (let j = 6; j < 8; j++) initialGrid[i][j] = 'red';
    ...
    if (isAnimating) {
      interval = setInterval(() => {
        setGrid((prevGrid) => {
          ...
          // Move the red columns
          for (let i = 0; i < 10; i++) {
            const redColumn = newGrid[i].splice(6, 2); // Remove the red columns
            newGrid[i] = [...redColumn, ...newGrid[i]]; // Add them to the beginning
            if (newGrid[i][0] === 'red') {
              newGrid[i][0] = null; // Ensure the first two columns remain green
              newGrid[i][1] = null;
            }
          }
          return newGrid;
